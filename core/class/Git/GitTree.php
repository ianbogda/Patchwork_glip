...
class GitTree extends GitObject
{
	...
	public function _serialize()
	{
		...
		uasort($this->nodes, array('Patchwork\Git\GitTree', 'nodecmp'));
	...
	public function find($path)
	{
		...
		array_shift($path);
		while ($path && !$path[0]) array_shift($path);
		...
		else
		{
			...
			if (!($cur instanceof GitTree)) throw new GitTreeInvalidPathError;
	...
	public function updateNode($path, $mode, $object)
	{
		...
		else
		{
			...
			if (isset($this->nodes[$name]))
			{
				...
				if (!$node->is_dir) throw new GitTreeInvalidPathError;
	...
	static public function treeDiff($a_tree, $b_tree)
	{
		...
		while ($a < count($a_files) || $b < count($b_files))
		{
			...
			if ($b >= count($b_files) || 0 > $cmp)
			{
				$changes[$a_files[$a]] = self::TREEDIFF_REMOVED;
				++$a;
			}
			else if ($a >= count($a_files) || 0 < $cmp)
			{
