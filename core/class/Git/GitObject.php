...
class GitObject extends GitItem
{
	...
	{
		if (GitRepository::OBJ_COMMIT == $type) return new GitCommit($repo);
		if (GitRepository::OBJ_TREE   == $type) return new GitTree  ($repo);
		if (GitRepository::OBJ_BLOB   == $type) return new GitBlob  ($repo);
